...
*Comment utiliser Lytos avec Cursor IDE. Ce guide explique comment configurer un projet pour que Cursor charge le manifest, la memory, les skills et les rules.*
...
3. .lytos/rules/default-rules.md — les critères qualité

## Quand tu travailles sur une tâche

1. Demande quelle issue est en cours
2. Lis l'issue dans .lytos/issue-board/3-in-progress/
3. Charge le skill correspondant depuis .lytos/skills/
...
- .lytos/skills/code-review.md — comment faire une code review
- .lytos/skills/testing.md — comment écrire des tests
- .lytos/skills/documentation.md — comment documenter
- .lytos/skills/git-workflow.md — conventions Git
- .lytos/skills/code-structure.md — structure et nommage

## Après chaque tâche

- Mets à jour le status de l'issue si demandé
- Propose de mettre à jour .lytos/memory/MEMORY.md si un apprentissage a eu lieu
```
...
```
Refactore cette fonction en suivant les règles de .lytos/skills/code-structure.md
```
...
```
Lis .lytos/issue-board/3-in-progress/ISS-0012-titre.md et implémente 
la solution proposée en chargeant le skill .lytos/skills/code-structure.md
```

### Composer (Cmd+I)

Pour les modifications multi-fichiers, Composer est idéal avec Lytos :

```
En suivant .lytos/skills/testing.md, écris les tests unitaires 
...
Cursor permet de référencer des fichiers avec `@`. Utilisez cette fonctionnalité pour charger les fichiers de Lytos :

```
@.lytos/manifest.md @.lytos/memory/MEMORY.md
...
- **Utiliser `@` pour charger les fichiers de Lytos** à la demande plutôt que tout mettre dans `.cursorrules`
...
*Cet adapter fait le pont entre Cursor et Lytos. La méthode reste la même — seul le chargement est adapté à l'outil.*
